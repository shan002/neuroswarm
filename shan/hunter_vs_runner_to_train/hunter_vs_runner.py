...
def run(app, args):
    ...
    if getattr(args, 'plot_fit', False) or getattr(args, 'save_plot', False):
        plt.figure()
        ...
        if getattr(args, 'save_plot', False):
            out_dir = os.path.join('./plots', app.p.name)
            os.makedirs(out_dir, exist_ok=True)
            img_name = f"fitness_over_{args.trials}_trials_N_{args.agents}.png"
...
def get_parsers(parser, subpar):
    ...
    for sub in sp.values():
        ...
        sub.add_argument('--network', help="network folder path")
...
# python hunter_vs_runner.py run --root ~/neuromorphic/turtwig/results_sim/hopper/250525/farp/6-wtp/ --cy 3000 -T 2 --trial_seed 410 --world_yaml world_bin.yaml
